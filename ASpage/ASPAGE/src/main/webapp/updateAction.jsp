...
//작성한 글이 작성한 사람 본인인지 확인한다. 현재 넘어온 bbsID 값과 세션값을 비교한다
		...
		//리퀘스트 파라미터로 넘어오는 update에서 보내준 타이틀과 콘텐츠값이 null이거나 빈칸으로 넘어왔을때 돌려보낸다
			...
			|| request.getParameter("bbsTitle").equals("")|| request.getParameter("bbsTitle").equals("")){
				...
				int result = bbsDAO.update(bbsID,request.getParameter("bbsTitle"),request.getParameter("bbsContent"));//자바bean 6개를 모아둔 user로 register 호출
				
				if(result == -1){ // -1이 반환된다면 오류이다
					PrintWriter script = response.getWriter(); //
					script.println("<script>");
					script.println("alert('글수정이 실패했습니다.')"); //알림이 나온다
					script.println("history.back()");//이전 페이지로 돌려보낸다 (로그인페이지)
					script.println("</script>");
				}
					...
					PrintWriter script = response.getWriter();
					script.println("<script>");
					script.println("location.href = 'bbs.jsp'"); //메인페이지로 이동
					script.println("</script>");
				}
			};	
		}
